...
THEORAPLAY_Decoder *videoDecoder       = NULL;
const THEORAPLAY_VideoFrame *videoVidData = NULL;
THEORAPLAY_Io videoCallbacks;
...
static bool videoSkipped = false;
...
void PlayVideoFile(char *filePath)
{
    ...
    for (int c = 0; c < (int)strlen(pathBuffer); ++c) {
    ...
    if (file) {
        ...
        videoCallbacks.read     = videoRead;
        videoCallbacks.close    = videoClose;
        videoCallbacks.userdata = (void *)file;
#if RETRO_USING_SDL2 && !RETRO_USING_OPENGL
        videoDecoder =
            THEORAPLAY_startDecode(&videoCallbacks, 30, THEORAPLAY_VIDFMT_IYUV, GetGlobalVariableByName("Options.Soundtrack") ? 1 : 0);
#endif

        // TODO: does SDL1.2 support YUV?
#if RETRO_USING_SDL1 && !RETRO_USING_OPENGL
        videoDecoder =
            THEORAPLAY_startDecode(&videoCallbacks, 30, THEORAPLAY_VIDFMT_RGBA, GetGlobalVariableByName("Options.Soundtrack") ? 1 : 0);
#endif
